...
WOQLQuery.prototype.node = function(node, type) {
    ...
    if (type == 'delete_quad') type == 'DeleteQuad'
    if (type == 'add_triple') type == 'AddTriple'
    if (type == 'delete_triple') type == 'DeleteTriple'
    if (type == 'quad') type == 'Quad'
    if (type == 'triple') type == 'Triple'
...
WOQLQuery.prototype._get_object = function(s, p) {
    if (this.cursor['@type'] == 'woql:And') {
        for (var i = 0; i < this.cursor['query_list'].length; i++) {
            ...
            if (
                this._same_entry(subq.query['woql:subject'], s) &&
                this._same_entry(subq.query['woql:predicate'], p)
            )
...
WOQLQuery.prototype._card = function(n, which) {
    ...
    let newid = s + '_' + which
    this.add_quad(newid, 'type', 'owl:Restriction', g).add_quad(newid, 'owl:onProperty', s, g)
    switch (which) {
        case 'max':
            this.add_quad(
                newid,
