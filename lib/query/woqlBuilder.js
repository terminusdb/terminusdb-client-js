...
WOQLQuery.prototype.lib = function(values, variables, resource) {
    return new WOQLLibrary(values, variables, resource)
}
...
WOQLQuery.prototype.abstract = function(graph, subj) {
    this._add_partial(subj, 'terminus:tag', 'terminus:abstract', graph)
    return this
}

WOQLQuery.prototype.node = function(node, type) {
    type = type || "woql:AddTriple"
    this._set_context({subject: node, action: type})
    return this
}
...
WOQLQuery.prototype.property = function(proId, type_or_value) {
    if (this._adding_class()) {
        ...
        this.and(
            new WOQLSchema().add_property(proId, type_or_value, g).domain(this._adding_class())
        )
    } else {
...
WOQLQuery.prototype.parent = function(...parentList) {
    for (var i = 0; i < parentList.length; i++) {
        ...
        this._add_partial('rdfs:subClassOf', pn)
    }
    return this
}
...
WOQLQuery.prototype._add_partial = function(s, p, o, g) {
    ...
    if (t == 'woql:Triple') this.triple(s, p, o)
    else if (t == 'woql:AddTriple') this.add_triple(s, p, o)
    else if (t == 'woql:DeleteTriple') this.delete_triple(s, p, o)
    else if (t == 'woql:Quad') this.quad(s, p, o, g)
    else if (t == 'woql:AddQuad') this.add_quad(this.subject, p, o, g)
    else if (t == 'woql:DeleteQuad') this.delete_quad(s, p, o, g)
    else if (g) this.quad(s, p, o, g)
    else this.triple(s, p, o)
    return this
}
...
WOQLQuery.prototype._adding_class = function(string_only) {
    ...
    if(x && string_only && typeof x == "object") x = x['woql:node'] 
    return x
}
...
WOQLQuery.prototype._set_context = function(ctxt) {
    if(!this.triple_builder_context) this.triple_builder_context = ctxt 
    else {
        for(var k in ctxt){
...
WOQLQuery.prototype._get_object = function(s, p) {
    if (this.cursor['@type'] == 'woql:And') {
        for (var i = 0; i < this.cursor['query_list'].length; i++) {
            ...
            if (this._same_entry(subq.query['woql:subject'], s) && this._same_entry(subq.query['woql:predicate'], p))
...
WOQLQuery.prototype._string_matches_object = function(s, o){
    ...
    if(o['woql:variable_name']) return s == o['woql:variable_name']['@value']
    return false
}

WOQLQuery.prototype.card = function(n, which) {
    ...
    let newid = s + "_" + which
    this.add_quad(newid, "type", 'owl:Restriction', g)
        .add_quad(newid,'owl:onProperty', s, g)
    switch (which) {
        case 'max':
            this.add_quad(newid, 'owl:maxCardinality', {'@value': n, '@type': 'xsd:nonNegativeInteger'}, g)
            break
        case 'min':
            this.add_quad(newid, 'owl:minCardinality', {'@value': n, '@type': 'xsd:nonNegativeInteger'}, g)
            break
        default:
            this.add_quad(newid, 'owl:cardinality', {'@value': n, '@type': 'xsd:nonNegativeInteger'}, g)
    }
    //then make the domain of the property into a subclass of the restriction
    let od = this._get_object(s, 'rdfs:domain')
    if (od) {
        this.add_quad(od, "subClassOf", newid, g) 
    }
    return this
}
