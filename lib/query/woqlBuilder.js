...
TripleBuilder.prototype.addPO = function(p, o, g) {
    ...
    else if (this.type == 'woql:DeleteTriple')
        newq = new WOQLQuery().delete_triple(this.subject, p, o)
    ...
    else if (this.type == 'woql:DeleteQuad')
        newq = new WOQLQuery().delete_quad(this.subject, p, o, g)
...
TripleBuilder.prototype.getO = function(s, p) {
    if (this.cursor['@type'] == 'woql:And') {
        for (var i = 0; i < this.cursor['query_list'].length; i++) {
            ...
            if (subq.query['woql:subject'] == s && subq.query['woql:predicate'] == p)
...
TripleBuilder.prototype.card = function(n, which) {
    ...
    let prop = this.subject
    if(typeof prop == "object"){
        if(prop.node) prop = prop.node
        else return
    }
    let newid = prop + "_" + which
    this.query.and(
        new WOQLQuery().add_quad(newid, "type", 'owl:Restriction', this.g)
        .add_quad(newid,'owl:onProperty', this.subject, this.g)
    )
    switch (which) {
        case 'max':
            this.query.and().add_quad(newid, 'owl:maxCardinality', {'@value': n, '@type': 'xsd:nonNegativeInteger'}, this.g)
            break
        case 'min':
            this.query.and().add_quad(newid, 'owl:minCardinality', {'@value': n, '@type': 'xsd:nonNegativeInteger'}, this.g)
            break
        default:
            this.query.and().add_quad(newid, 'owl:cardinality', {'@value': n, '@type': 'xsd:nonNegativeInteger'}, this.g)
    }
    //then make the domain of the property into a subclass of the restriction
    let od = this.getO(prop, 'rdfs:domain')
    if (od) {
        this.query.and().add_quad(od, "subClassOf", newid, this.g) 
    }
    return this
}
