...
WOQLQuery.prototype._sg = function(g) {
    if (g) return g
    if (this.triple_builder_context && this.triple_builder_context.graph)
        return this.triple_builder_context.graph
 ...
 * @property {array|string} [parent] -if not parent the new class will be a class Object
 * @property {array} [properties] - a class list of property
 */
 ...
 * @param {ClassObj} data with id, label, description, parent and properties
 * @param {string} graph - the resource identifier of a schema graph. The Default value is schema/main
 * @returns {WOQLQuery} 
 */
//KEVIN 
...
WOQLQuery.prototype.genEnum = function(prop, cls, clslabel, clsdesc, graph) {
    ...
    this.and(
        new WOQLQuery()
            .select('v:PV')
            .distinct('v:PV')
            .triple('v:A', prop, 'v:PV'),
        new WOQLQuery().idgen(cls, ['v:PV'], 'v:ChoiceID'),
        add_class,
        new WOQLQuery()
            .add_quad('v:ChoiceID', 'type', cls, graph)
...
WOQLQuery.prototype.makeEnum = function(client, prop, cls, clslabel, clsdesc, graph) {
    client
        .query(new WOQLQuery().genEnum(prop, cls, clslabel, clsdesc, graph))
...
WOQLQuery.prototype.boxDatatype = function(datatype, label, descr, parent, graph, prefix) {
    ...
    var box_class = new WOQLQuery()
        .add_class(box_class_id, graph)
        .label(label)
        .description('Boxed Class for ' + datatype)
    if (parent) box_class.parent(parent)
    var box_prop = new WOQLQuery()
        .add_property(box_prop_id, datatype, graph)
        .label(label)
        .domain(box_class_id)
...
WOQLQuery.prototype.boxClasses = function(prefix, classes, except, graph) {
    ...
    let idgens = [
        ...
        new WOQLQuery()
            .concat([prefix, 'v:Cname'], 'v:Class')
            .cast('v:Class', 'owl:Thing', 'v:ClassID'),
        new WOQLQuery()
            .concat([prefix, 'v:Propname'], 'v:Prop')
            .cast('v:Prop', 'owl:Thing', 'v:PropID'),
    ]
    ...
    let cls = new WOQLQuery()
        .graph(graph)
        .add_class('v:ClassID')
        .label('v:Label')
        .description('v:CDesc')
    let prop = new WOQLQuery()
        .graph(graph)
