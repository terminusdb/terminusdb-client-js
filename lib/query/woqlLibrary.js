...
WOQLLibrary.prototype.getEverything = function(Graph, Subj, Pred, Obj){
	Subj = Subj || "v:Subject"
	Pred = Pred || "v:Predicate"
	Obj = Obj  ||  "v:Object"
	Graph = Graph || false;
	if(Graph){
		return new WOQLQuery().quad(Subj, Pred, Obj, Graph);
	}
	else {
		return new WOQLQuery().triple(Subj, Pred, Obj);
	}
}
