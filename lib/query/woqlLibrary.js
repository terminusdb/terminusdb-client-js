...
WOQLLibrary.prototype.active_commit = function(branch, ts) {
    let [biri, hiri, ciri, cpath, cts, tiri, tts, cid, hts] = this
        .vars("My Branch IRI", "My Head IRI", "My Child IRI", "Commit Path", "My Child Timestamp", "My Tail IRI", "My Tail Timestamp", "Commit ID",  "My Head TS" )
    let q = new WOQLQuery().limit(1).and(
        new WOQLQuery().using("_commits").limit(1).select(cid)
...
/**
 * Retrieves the next (child) commit to the passed one on a particular branch
 */

WOQLLibrary.prototype.next_commit = function(commit_id, branch) {
    let [branch_iri, head_iri, commit_iri, cpath, tail_iri, cid] = this
        .vars("My Branch IRI", "My Head IRI", "My Child IRI", "Commit Path", "My Tail IRI", "Commit ID" )
    let q = new WOQLQuery().limit(1).and(
        new WOQLQuery().using("_commits").limit(1).select(cid)
        ...
        .or(
            new WOQLQuery().triple(head_iri, "ref:commit_parent", tail_iri)
            .triple(head_iri, "ref:commit_id", cid),
            new WOQLQuery().path(head_iri, "ref:commit_parent+", commit_iri, cpath)
            .triple(commit_iri, "ref:commit_parent", tail_iri)
