...
WOQLLibrary.prototype.active_commit_id = function(branch, ts, commit_var) {
    ...
    let q = new WOQLQuery()
        ...
        .limit(1)
        .select(cid)
        .triple(biri, 'name', {'@type': 'xsd:string', '@value': branch})
        .triple(biri, 'head', hiri)
        .or(
            new WOQLQuery()
                .triple(hiri, 'timestamp', hts)
                .triple(hiri, 'identifier', cid)
                .not()
                .greater(hts, ts),
            new WOQLQuery()
                .or(
                    new WOQLQuery()
                        .path(hiri, 'parent*', ciri, cpath)
                        .triple(ciri, 'parent', tiri),
                    new WOQLQuery().triple(ciri, 'parent', tiri),
                )
                .triple(ciri, 'timestamp', cts)
                .triple(tiri, 'timestamp', tts)
                .triple(tiri, 'identifier', cid)
                .greater(cts, ts)
                .not()
                .greater(tts, ts),
        )
