...
WOQLLibrary.prototype.commits = function(values, variables, cresource) {
    ...
    let qpattern = new WOQLQuery()
        ...
        .limit(1).or(
            ...
            new WOQLQuery().eq(this._varn('Parent IRI'), "").eq(this._varn('Parent ID'), "")
        )
        .limit(1).or(
            new WOQLQuery()
...
WOQLLibrary.prototype.dbs = function(values, variables) {
    ...
    let compiled = this._add_constraints(woql, values)
    return new WOQLQuery().using("terminus", compiled)
} 
...
/**
 * Below are mostly just simple wrappers around the above
 */
...
WOQLLibrary.prototype.insert_prefix = function(values, variables, cresource) {
    ...
    let q = TerminusClient.WOQL.using(cresource)
        .idgen('doc:PrefixPair', values, this._var(0))
        .add_triple('ref:default_prefixes', 'ref:prefix_pair', this._var(0))
        .insert(this._var(0), 'ref:PrefixPair')
