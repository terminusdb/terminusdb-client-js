...
WOQLLibrary.prototype.add_multiple = async function(client, parts, varlist, clause, commit){
    let p = new WOQLQuery()
...
WOQLLibrary.prototype.asset_overview = function(using, v){
    ...
    q.and(
        new WOQLQuery().using(using).size(using, v[0]),
        new WOQLQuery().using(meta).opt().triple(v[1], "repo:remote_url", v[2]),
        new WOQLQuery().using(commits).opt().triple(v[7], 'ref:ref_commit', v[8])
        .triple(v[8], 'ref:schema', v[9])
    )
    q.opt().and().using(commits).limit(1).order_by(v[3])
        .triple(v[4], "ref:commit_timestamp", v[3])
        .and(
            new WOQLQuery().opt().triple(v[4], "ref:commit_author", v[5]),
            new WOQLQuery().opt().triple(v[4], "ref:commit_message", v[6]),
            new WOQLQuery().opt().triple(v[10], "ref:commit_timestamp", v[11])
                .not().triple(v[10], 'ref:commit_parent', v[12])
        )
    return q
}
...
WOQLLibrary.prototype.assets_overview = async function(dbs, client){
    let vars = ['v:size', 'v:RemoteIRI', "v:remote", 'v:updated', 'v:CommitIRI', 'v:author', 
    'v:message', "v:BranchIRI", 'v:CommitIRI', "v:schema", "v:C2IRI", "v:created", "v:NOParent"] 
    let res = await this.add_multiple(client, dbs, vars, this.asset_overview)
    let dbmetas = []
    for(var i = 0; i<dbs.length; i++){
        ...
        if(rec){
            ...
            if(rec['remote']&& rec['remote']['@value']) dbmeta.remote = rec['remote']['@value']
