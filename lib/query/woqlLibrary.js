...
WOQLLibrary.prototype.add_remote = function(meta_resource, remote_url, name){
    ...
    return new WOQLQuery().using(meta_resource).into("instance/main").and(
        ...
        new WOQLQuery().add_triple("v:Remote", "type", "repo:Remote")
            .add_triple("v:Remote", "repo:repository_name", name)
            .add_triple("v:Remote", "repo:remote_url", remote_url)
    )
}
...
WOQLLibrary.prototype.asset_overview = function(using, v){
    ...
    let cs = []
    cs.push(new WOQLQuery().opt().limit(1).triple(v[3], 'ref:schema', v[4]))
    cs.push(new WOQLQuery().opt().limit(1).triple(v[20], "ref:commit_timestamp", v[22]).not().triple(v[20], 'ref:commit_parent', v[21]))
    cs.push(new WOQLQuery().opt().limit(1).order_by(v[15], "desc", v[17])
        .triple(v[16], "ref:commit_timestamp", v[15])
        .and(
            new WOQLQuery().opt().triple(v[16], "ref:commit_author", v[17]),
            new WOQLQuery().opt().triple(v[16], "ref:commit_message", v[18]),
            new WOQLQuery().triple(v[16], "ref:commit_id", v[19]),
        )
    )        
    cs.push(new WOQLQuery().opt().group_by(v[5], [v[5], v[6]], v[8])
        .triple('ref:default_prefixes', 'ref:prefix_pair', v[7])
        .triple(v[7], 'ref:prefix', v[5])
        .triple(v[7], 'ref:prefix_uri', v[6])
    )
    cs.push(new WOQLQuery().opt().group_by(v[10], [v[10], v[12], v[13]], v[14])
        .triple(v[9], 'ref:branch_name', v[10])
        .triple(v[9], 'ref:ref_commit', v[11])
        .triple(v[11], "ref:commit_id", v[12])
        .triple(v[11], "ref:commit_timestamp", v[13])
    )
    q.and(
        //new WOQLQuery().using(using).opt().size(using, v[0]),
        new WOQLQuery().using(meta).opt().limit(1).triple(v[1], "repo:remote_url", v[2]),
        new WOQLQuery().using(commits).and(...cs),
    )
    return q
}
/*
new WOQLQuery().opt().using(commits).select(v[14]).group_by(v[10], [v[10], v[12], v[13]], v[14])
    .triple(v[9], 'ref:branch_name', v[10])
    .triple(v[9], 'ref:ref_commit', v[11])
    .triple(v[11], "ref:commit_id", v[12])
    .triple(v[11], "ref:commit_timestamp", v[13]),
...
//WOQL.lib().assets_overview(['admin/SaAS'])

WOQLLibrary.prototype.assets_overview = function(dbs, client){
    let vars = ['v:size', 'v:RemoteIRI', "v:remote_url", "v:AnyCommit", "v:schema", 'v:prefix', 'v:prefix_URI', 'v:PrefixPairIRI', "v:prefixes", 
     'v:BranchIRI', 'v:branch', 'v:BranchHeadIRI', 'v:branch_head', 'v:branch_time', 'v:branches', 'v:updated', 
     'v:LatestCommitIRI', 'v:author', 'v:message', 'v:latest_commit', 'v:FirstCommitIRI', 'v:NoParent', 'v:created']
    return this.add_multiple(client, dbs, vars, this.asset_overview)
    .then((res) => {
        let dbmetas = []
        for(var i = 0; i<dbs.length; i++){
            let dbmeta = {organization: dbs[i].substring(1, dbs[i].lastIndexOf("/")), id: dbs[i].substring(dbs[i].lastIndexOf("/") + 1)}
            let rec = res[i]
            if(rec){
                if(rec['author']&& rec['author']['@value']) dbmeta.author = rec['author']['@value']
                if(rec['message']&& rec['message']['@value']) dbmeta.message = rec['message']['@value']
                if(rec['updated']&& rec['updated']['@value']) dbmeta.updated = rec['updated']['@value']
                if(rec['created']&& rec['created']['@value']) dbmeta.created = rec['created']['@value']
                if(rec['latest_commit']&& rec['latest_commit']['@value']) dbmeta.latest_commit = rec['latest_commit']['@value']
                ...
                if(rec['branches'] && rec['branches'] != "system:unknown") dbmeta.branches = unWindBranches(rec['branches'])
            }
            dbmetas.push(dbmeta)
        }
        return dbmetas
