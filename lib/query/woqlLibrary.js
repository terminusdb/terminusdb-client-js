...
WOQLLibrary.prototype.asset_overview = function(using, v){
    ...
    cs.push(new WOQLQuery().opt().group_by(v[10], [v[10], v[12], v[13], v[14], v[15]], v[16])
        ...
        .triple(v[11], "ref:commit_author", v[14])
        .triple(v[11], "ref:commit_message", v[15])
    )
    cs.push(new WOQLQuery().opt().limit(1).triple(v[17], "ref:commit_timestamp", v[19]).not().triple(v[20], 'ref:commit_parent', v[18]))
    q.and(
...
function unWindBranches(branches){
    ...
    for(var i = 0; i<branches.length; i++){
        ...
        if(branches[i][2]['@value'] > parts.updated){
            ...
            parts.author = branches[i][4]['@value']
        }
    }
    parts.branches = bs 
    return parts
