...
//KEVIN THIS IS NO POSSIBLE  WE DON@T USE THIS QUAD IS NOT DECLARED
/**
...
//KEVIN THE AND FUNCTION DOSEN'T EXIST HERE IT IS IN woqlQuery FILE
...
WOQLQuery.prototype.findLastProperty = function(json) {
    ...
    if (
        json &&
        json['woql:subject'] &&
        this._is_property_triple(json['woql:predicate'], json['woql:object'])
    ) {
        return json
    }
    return false
...
function compilePredicate(pp, q) {
    if (pp.indexOf('<') != -1 && pp.indexOf('>') != -1) {
        ...
        let cleaned = pred == '*' ? 'owl:topObjectProperty' : q.cleanPathPredicate(pred)
        return {'@type': 'woql:InvertedPathPredicate', 'woql:path_predicate': {'@id': cleaned}}
    } else if (pp.indexOf('>') != -1) {
