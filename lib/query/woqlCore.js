...
WOQLQuery.prototype.asv = function(colname_or_index, vname, type) {
    ...
    if (typeof colname_or_index == 'number') {
        asvar['@type'] = 'IndexedAsVar'
        asvar['index'] = this.jlt(colname_or_index, 'xsd:nonNegativeInteger')
    } else if (typeof colname_or_index == 'string') {
        asvar['@type'] = 'NamedAsVar'
        asvar['identifier'] = this.jlt(colname_or_index)
    }
    if (vname.substring(0, 2) == 'v:') vname = vname.substring(2)
    asvar['variable_name'] = {'@type': 'xsd:string', '@value': vname}
    if (type) asvar['var_type'] = this.jlt(type, 'xsd:anyURI')
    return asvar
}
...
WOQLQuery.prototype.cleanObject = function(o, t) {
    ...
    if (typeof o === 'string') {
        if (this.looksLikeClass(o)) {
            ...
            obj['datatype'] = this.jlt(o, t)
        }
    } else if (typeof o == 'number') {
        t = t || 'xsd:decimal'
        obj['datatype'] = this.jlt(o, t)
    } else if (typeof o == 'object' && o) {
        if (o['@value']) obj['datatype'] = o
        else return o
    }
    return obj
}
...
WOQLQuery.prototype.findLastSubject = function(json) {
    if (json && json['query_list']) {
        for (var i = json['query_list'].length - 1; i >= 0; i--) {
            let lqs = this.findLastSubject(json['query_list'][i])
            if (lqs) return lqs
        }
    }
    if (json && json['query']) {
...
function compilePredicate(pp, q) {
    if (pp.indexOf('<') != -1 && pp.indexOf('>') != -1) {
        ...
        let cleaned = pred == '*' ? 'owl:topObjectProperty' : q.cleanPathPredicate(pred)
        return {'@type': 'InvertedPathPredicate', 'path_predicate': {'@id': cleaned}}
    } else if (pp.indexOf('>') != -1) {
        let pred = pp.slice(0, pp.length - 1)
        let cleaned = pred == '*' ? 'owl:topObjectProperty' : q.cleanPathPredicate(pred)
        return {'@type': 'PathPredicate', 'path_predicate': {'@id': cleaned}}
    } else {
        let pred = pp == '*' ? 'owl:topObjectProperty' : q.cleanPathPredicate(pp)
        return {'@type': 'PathPredicate', 'path_predicate': {'@id': pred}}
    }
}
...
function tokensToJSON(seq, q) {
    if (seq.length == 1) {
        ...
        return {
            '@type': 'PathOr',
            'path_left': tokensToJSON(left, q),
            'path_right': tokensToJSON(right, q),
        }
    } else if (seq.indexOf(',') != -1) {
        //binds tighter
        ...
        return {
            '@type': 'PathSequence',
            'path_first': tokensToJSON(first, q),
            'path_second': tokensToJSON(second, q),
        }
    } else if (seq[1] == '+') {
        //binds tightest of all
        return {
            '@type': 'PathPlus',
            'path_pattern': tokensToJSON([seq[0]], q),
        }
        ...
        return {
            '@type': 'PathTimes',
            'path_minimum': {'@type': 'xsd:positiveInteger', '@value': meat[0]},
            'path_maximum': {'@type': 'xsd:positiveInteger', '@value': meat[1]},
            'path_pattern': tokensToJSON([seq[0]], q),
        }
    } else {
