...
WOQLQuery.prototype.and = function(...queries) {
    ...
    for (let i = 0; i < queries.length; i++) {
        ...
        if (
            ...
            for (var j = 0; j < onevar['and'].length; j++) {
                let qjson = onevar['and'][j]
                if (qjson) {
                    let subvar = this.jobj(qjson)
...
WOQLQuery.prototype.update_object = function(docjson) {
    ...
    this.cursor['document'] =
        typeof docjson == 'string' ? this.expandValue(docjson, true) : docjson
    return this.updated()
}
...
WOQLQuery.prototype.delete_object = function(JSON_or_IRI) {
    ...
    this.cursor['document_uri'] = this.expandValue(JSON_or_IRI)
    return this.updated()
}
...
WOQLQuery.prototype.read_object = function(IRI, OutputVar, Format) {
    ...
    this.cursor['document_uri'] = this.expandValue(IRI)
    this.cursor['document'] = this.expandValue(OutputVar)
