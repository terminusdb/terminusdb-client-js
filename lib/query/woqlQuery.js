 ...
 * WOQL.using("userName/dbName/local/commit|branch/commitID|branchname").triple(a, b, c)
 */
 ...
 * // Matches when the variables are set, and queries variables that are not set
 */
 ...
 *   literal(100, "integer")
 * );
 * // Queries the range of values with very high performance thanks to succinct data structures
 */
 ...
 * );
 * // Queries the range of values with very high performance thanks to succinct data structures,
 * and retrieves in reverse order
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * triple_next("v:subject", "v:predicate", "v:object", "v:next");
 * // This is very helpful for navigating the stored triples fast
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * quad_next("v:subject", "v:predicate", "v:object", "v:next", "v:graph");
 * // This is very helpful for navigating the stored triples fast in a graph
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * triple_previous("v:subject", "v:predicate", "v:object", "v:previous");
 * // This is very helpful for navigating the stored triples backwards
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * quad_previous("v:subject", "v:predicate", "v:object", "v:previous", "v:graph");
 * // This is very helpful for navigating the stored triples backwards in a graph
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * added_triple("v:subject", "v:predicate", "v:object");
 * // This helps to find which triples were added to the currently stored layer
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * removed_triple("v:subject", "v:predicate", "v:object");
 * // This helps to find which triples were removed from the currently stored layer
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * quad("v:subject", "v:predicate", "v:object", "v:graph");
 * // This helps to find triples in a specific graph (such as the "schema" graph)
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * added_quad("v:subject", "v:predicate", "v:object", "v:graph");
 * // This helps to find which quads were added to the currently stored layer
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * removed_quad("v:subject", "v:predicate", "v:object", "v:graph");
 * // This helps to find which quads were removed from the currently stored layer
 */
 ...
 * @example
 * sub("v:classA", "v:classB");
 * // This helps to find if classA subsumes classB (classA is more specific than class B)
 */
 ...
 * @returns {WOQLQuery}
 * @example
 * eq("v:varName", "v:varValue");
 * // This helps to match if varName is equal to varValue
 */
 ...
 * get(as("Name", "v:name").as("Label", "v:label")).post("file.csv",{"type":"csv"}, "post")
 * // map the column Name to variable name, Label>label, from the attached csv file "file.csv"
 */
...
/**
 * Identifies a resource attached to the POST request or
 * a local path on the client, to be sent to the server through a
 ...
 * @example
 * let [result, number] = vars("result", "number")
 * and(
 *   eq(number, 1.1),
 *   less(number, 1.2).eq(result, literal(true, "boolean"))
 * )
 * //result contains true
 */
WOQLQuery.prototype.less = function (varNum01, varNum02) {
  if (typeof varNum01 === 'undefined' || typeof varNum02 === 'undefined') return this.parameterError('Less takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
 ...
 * and(
 *   eq(number, 1.2)
 *   greater(number, 1.1).eq(result, literal(true, "boolean")),
 * )
 * //result contains true
 */
WOQLQuery.prototype.greater = function (varNum01, varNum02) {
  if (typeof varNum01 === 'undefined' || typeof varNum02 === 'undefined') return this.parameterError('Greater takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
...
/**
 * Alias for greater — strict greater-than comparison.
 * @param {string|number|Var} left - The greater value
 * @param {string|number|Var} right - The lesser value
 * @returns {WOQLQuery}
 * @example
 * let [result, number] = vars("result", "number")
 * and(
 *   eq(number, 1.2),
 *   gt(number, 1.1).eq(result, literal(true, "boolean"))
 * )
 */
WOQLQuery.prototype.gt = WOQLQuery.prototype.greater;

/**
 * Alias for less — strict less-than comparison.
 * @param {string|number|Var} left - The lesser value
 * @param {string|number|Var} right - The greater value
 * @returns {WOQLQuery}
 * @example
 * let [result, number] = vars("result", "number")
 * and(
 *   eq(number, 1.1),
 *   lt(number, 1.2).eq(result, literal(true, "boolean"))
 * )
 */
WOQLQuery.prototype.lt = WOQLQuery.prototype.less;
 ...
 * and(
 *   eq(number, 1.2),
 *   gte(number, 1.1).eq(result, literal(true, "boolean"))
 * )
 */
WOQLQuery.prototype.gte = function (left, right) {
  if (typeof left === 'undefined' || typeof right === 'undefined') return this.parameterError('Gte takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'Gte';
  this.cursor.left = this.cleanDataValue(left);
  this.cursor.right = this.cleanDataValue(right);
  return this;
};
 ...
 * and(
 *   eq(number, 1.1),
 *   lte(number, 1.2).eq(result, literal(true, "boolean"))
 * )
 */
WOQLQuery.prototype.lte = function (left, right) {
  if (typeof left === 'undefined' || typeof right === 'undefined') return this.parameterError('Lte takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'Lte';
  this.cursor.left = this.cleanDataValue(left);
  this.cursor.right = this.cleanDataValue(right);
  return this;
};
 ...
 * and(
 *   eq(number, 1.1),
 *   in_range(number, 1.0, 2.0).eq(result, literal(true, "boolean"))
 * )
 */
WOQLQuery.prototype.in_range = function (value, start, end) {
  if (typeof value === 'undefined' || typeof start === 'undefined' || typeof end === 'undefined') return this.parameterError('InRange takes three parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'InRange';
  this.cursor.value = this.cleanDataValue(value);
 ...
 * let [result] = vars("result")
 * and(
 *   sequence(number, 1.0, 2.0, 0.1).eq(result, literal(true, "boolean"))
 * )
 * // solutions where number as 1.0, 1.1, ... , 1.9 get generated
 */
WOQLQuery.prototype.sequence = function (value, start, end, step, count) {
  if (typeof value === 'undefined' || typeof start === 'undefined' || typeof end === 'undefined') return this.parameterError('Sequence takes at least three parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'Sequence';
  this.cursor.value = this.cleanDataValue(value);
  this.cursor.start = this.cleanDataValue(start);
  this.cursor.end = this.cleanDataValue(end);
  if (typeof step !== 'undefined') {
    this.cursor.step = this.cleanDataValue(step);
  }
  if (typeof count !== 'undefined') {
    this.cursor.count = this.cleanDataValue(count);
  }
  return this;
};
 ...
 * @returns {WOQLQuery}
 * @example
 * month_start_date("2025-01", "v:date");
 * // "v:..." indicates date is a variable
 * // date returns 2025-01-01
 */
WOQLQuery.prototype.month_start_date = function (yearMonth, date) {
  if (typeof yearMonth === 'undefined' || typeof date === 'undefined') return this.parameterError('MonthStartDate takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'MonthStartDate';
  this.cursor.year_month = this.cleanDataValue(yearMonth);
 ...
 * // date returns 2000-02-29
 */
WOQLQuery.prototype.month_end_date = function (yearMonth, date) {
  if (typeof yearMonth === 'undefined' || typeof date === 'undefined') return this.parameterError('MonthEndDate takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'MonthEndDate';
  this.cursor.year_month = this.cleanDataValue(yearMonth);
  this.cursor.date = this.cleanDataValue(date);
  return this;
};
 ...
 * @returns {WOQLQuery}
 * month_start_dates("v:date", literal("2000-01-01", "xsd:date"), literal("2001-01-01", "xsd:date"))
 */
WOQLQuery.prototype.month_start_dates = function (date, start, end) {
  if (typeof date === 'undefined' || typeof start === 'undefined' || typeof end === 'undefined') return this.parameterError('MonthStartDates takes three parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'MonthStartDates';
  this.cursor.date = this.cleanDataValue(date);
  this.cursor.start = this.cleanDataValue(start);
  this.cursor.end = this.cleanDataValue(end);
  return this;
 ...
 * @param {string|object|Var} end - The exclusive end date
 * @returns {WOQLQuery}
 * month_end_dates("v:date", literal("2000-01-01", "xsd:date"), literal("2001-01-01", "xsd:date"))
 * // 2000-01-31, 2000-02-29, ... ,  2000-12-31
 */
WOQLQuery.prototype.month_end_dates = function (date, start, end) {
  if (typeof date === 'undefined' || typeof start === 'undefined' || typeof end === 'undefined') return this.parameterError('MonthEndDates takes three parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'MonthEndDates';
  this.cursor.date = this.cleanDataValue(date);
  this.cursor.start = this.cleanDataValue(start);
  this.cursor.end = this.cleanDataValue(end);
  return this;
 ...
 * @returns {WOQLQuery}
 * interval_relation("v:rel", "2000-01-01", "2001-01-01", "2000-06-01", "2000-12-31")
 * // "contains"
 */
WOQLQuery.prototype.interval_relation = function (relation, xStart, xEnd, yStart, yEnd) {
  if (typeof relation === 'undefined' || typeof xStart === 'undefined' || typeof xEnd === 'undefined' || typeof yStart === 'undefined' || typeof yEnd === 'undefined') return this.parameterError('IntervalRelation takes five parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'IntervalRelation';
  this.cursor.relation = this.cleanDataValue(relation);
  this.cursor.x_start = this.cleanDataValue(xStart);
  this.cursor.x_end = this.cleanDataValue(xEnd);
  this.cursor.y_start = this.cleanDataValue(yStart);
  this.cursor.y_end = this.cleanDataValue(yEnd);
  return this;
};

/**
 * Allen's Interval Algebra on xdd:dateTimeInterval values.
 * Classifies or validates the temporal relationship between two interval values.
 * @param {string|object|Var} relation - Relation name (string) or variable for classification
 * @param {string|object|Var} x - First xdd:dateTimeInterval value
 * @param {string|object|Var} y - Second xdd:dateTimeInterval value
 * @returns {WOQLQuery}
 * interval_relation_typed(
 *   "v:rel",
 *   literal("2000-01-01T00:00:00/2001-01-01T00:00:00","xdd:dateTimeInterval"),
 *   literal("2000-06-01T00:00:00/2000-12-31T23:59:59", "xdd:dateTimeInterval")
 * )
 * // "contains"
 */
WOQLQuery.prototype.interval_relation_typed = function (relation, x, y) {
  if (typeof relation === 'undefined' || typeof x === 'undefined' || typeof y === 'undefined') return this.parameterError('IntervalRelationTyped takes three parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'IntervalRelationTyped';
  this.cursor.relation = this.cleanDataValue(relation);
  this.cursor.x = this.cleanDataValue(x);
  this.cursor.y = this.cleanDataValue(y);
  return this;
};
 ...
 * @param {Array|object|Var} list - The list of values
 * @param {string|object|Var} result - Variable or value for the minimum
 * @returns {WOQLQuery}
 * range_min([1, 2, 3], "v:min")
 * // "1"
 */
WOQLQuery.prototype.range_min = function (list, result) {
  if (typeof list === 'undefined' || typeof result === 'undefined') return this.parameterError('RangeMin takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'RangeMin';
  this.cursor.list = this.cleanDataValue(list);
  this.cursor.result = this.cleanDataValue(result);
  return this;
};
 ...
 * Works with any comparable types: numbers, dates, strings.
 * @param {Array|object|Var} list - The list of values
 * @param {string|object|Var} result - Variable or value for the maximum
 * @returns {WOQLQuery}
 * range_max([1, 2, 3], "v:max")
 * // "3"
 */
WOQLQuery.prototype.range_max = function (list, result) {
  if (typeof list === 'undefined' || typeof result === 'undefined') return this.parameterError('RangeMax takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'RangeMax';
  this.cursor.list = this.cleanDataValue(list);
  this.cursor.result = this.cleanDataValue(result);
  return this;
};
...
/**
 * Constructs or deconstructs a half-open xdd:dateTimeInterval.
 * @param {string|object|Var} start - Inclusive start date
 * @param {string|object|Var} end - Exclusive end date
 * @param {string|object|Var} interval - The interval value
 * @returns {WOQLQuery}
 * interval(literal("2000-01-01", "xsd:date"), literal("2001-01-01", "xsd:date"), "v:interval")
 * // "2000-01-01/2001-01-01"
 */
WOQLQuery.prototype.interval = function (start, end, interval) {
  if (typeof start === 'undefined' || typeof end === 'undefined'
      || typeof interval === 'undefined') {
    return this.parameterError('Interval takes three parameters');
  }
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'Interval';
  this.cursor.start = this.cleanDataValue(start);
  this.cursor.end = this.cleanDataValue(end);
  this.cursor.interval = this.cleanDataValue(interval);
  return this;
};

/**
 * Relates an xdd:dateTimeInterval to its start endpoint and precise xsd:duration.
 * @param {string|object|Var} start - Inclusive start date or dateTime
 * @param {string|object|Var} duration - The xsd:duration between start and end
 * @param {string|object|Var} interval - The xdd:dateTimeInterval value
 * @returns {WOQLQuery}
 * interval_start_duration(
 *   literal("2000-01-01", "xsd:date"),
 *   literal("P1Y", "xsd:duration"),
 *   "v:interval"
 * )
 * // "2000-01-01/2001-01-01"
 */
WOQLQuery.prototype.interval_start_duration = function (start, duration, interval) {
  if (typeof start === 'undefined' || typeof duration === 'undefined'
      || typeof interval === 'undefined') {
    return this.parameterError('IntervalStartDuration takes three parameters');
  }
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'IntervalStartDuration';
  this.cursor.start = this.cleanDataValue(start);
  this.cursor.duration = this.cleanDataValue(duration);
  this.cursor.interval = this.cleanDataValue(interval);
  return this;
};
 ...
 * @param {string|object|Var} duration - The xsd:duration between start and end
 * @param {string|object|Var} end - Exclusive end date or dateTime
 * @param {string|object|Var} interval - The xdd:dateTimeInterval value
 * @returns {WOQLQuery}
 * interval_duration_end(
 *   literal("P1Y", "xsd:duration"),
 *   literal("2001-01-01", "xsd:date"),
 *   "v:interval"
 * )
 * // "P1Y/2001-01-01"
 */
WOQLQuery.prototype.interval_duration_end = function (duration, end, interval) {
  if (typeof duration === 'undefined' || typeof end === 'undefined'
      || typeof interval === 'undefined') {
    return this.parameterError('IntervalDurationEnd takes three parameters');
  }
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'IntervalDurationEnd';
  this.cursor.duration = this.cleanDataValue(duration);
  this.cursor.end = this.cleanDataValue(end);
  this.cursor.interval = this.cleanDataValue(interval);
  return this;
 ...
 * @param {string|object|Var} date - The input date
 * @param {string|object|Var} next - The next calendar day
 * @returns {WOQLQuery}
 * day_after(literal("2000-02-28", "xsd:date"), "v:next")
 * // "2000-02-29"
 */
WOQLQuery.prototype.day_after = function (date, next) {
  if (typeof date === 'undefined' || typeof next === 'undefined') {
    return this.parameterError('DayAfter takes two parameters');
  }
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'DayAfter';
  this.cursor.date = this.cleanDataValue(date);
  this.cursor.next = this.cleanDataValue(next);
  return this;
};
 ...
 * @param {string|object|Var} date - The input date
 * @param {string|object|Var} previous - The previous calendar day
 * @returns {WOQLQuery}
 * day_before(literal("2000-03-01", "xsd:date"), "v:previous")
 * // "2000-02-29"
 */
WOQLQuery.prototype.day_before = function (date, previous) {
  if (typeof date === 'undefined' || typeof previous === 'undefined') {
    return this.parameterError('DayBefore takes two parameters');
  }
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'DayBefore';
  this.cursor.date = this.cleanDataValue(date);
  this.cursor.previous = this.cleanDataValue(previous);
  return this;
};

/**
 * Computes the ISO 8601 weekday number (Monday=1, Sunday=7) for a date.
 * Accepts xsd:date or xsd:dateTime.
 * @param {string|object|Var} date - The input date or dateTime
 * @param {string|number|Var} weekday - The ISO weekday number (1=Monday, 7=Sunday)
 * @returns {WOQLQuery}
 * weekday(literal("2000-02-29", "xsd:date"), "v:weekday")
 * // 2
 */
WOQLQuery.prototype.weekday = function (date, weekday) {
  if (typeof date === 'undefined' || typeof weekday === 'undefined') return this.parameterError('Weekday takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
 ...
 * Accepts xsd:date or xsd:dateTime.
 * @param {string|object|Var} date - The input date or dateTime
 * @param {string|number|Var} weekday - The US weekday number (1=Sunday, 7=Saturday)
 * @returns {WOQLQuery}
 * weekday_sunday_start(literal("2000-02-29", "xsd:date"), "v:weekday")
 * // 3
 */
WOQLQuery.prototype.weekday_sunday_start = function (date, weekday) {
  if (typeof date === 'undefined' || typeof weekday === 'undefined') return this.parameterError('WeekdaySundayStart takes two parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
 ...
 * Accepts xsd:date or xsd:dateTime. The ISO year may differ from calendar year at boundaries.
 * @param {string|object|Var} date - The input date or dateTime
 * @param {string|number|Var} year - The ISO week-numbering year
 * @param {string|number|Var} week - The ISO week number (1-53)
 * @returns {WOQLQuery}
 * iso_week(literal("2000-02-29", "xsd:date"), "v:year", "v:week")
 * // 2000, 9
 */
WOQLQuery.prototype.iso_week = function (date, year, week) {
  if (typeof date === 'undefined' || typeof year === 'undefined' || typeof week === 'undefined') return this.parameterError('IsoWeek takes three parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
 ...
 * @param {string|object|Var} start - The start date or dateTime
 * @param {string|object|Var} end - The end date or dateTime
 * @param {string|object|Var} duration - The xsd:duration between start and end
 * @returns {WOQLQuery}
 * date_duration(literal("2000-01-31", "xsd:date"), literal("2000-02-29", "xsd:date"), "v:duration")
 * // "P29D"
 */
WOQLQuery.prototype.date_duration = function (start, end, duration) {
  if (start === undefined || end === undefined || duration === undefined) return this.parameterError('DateDuration takes three parameters');
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'DateDuration';
  this.cursor.start = this.cleanDataValue(start);
  this.cursor.end = this.cleanDataValue(end);
  this.cursor.duration = this.cleanDataValue(duration);
  return this;
 ...
 * @example
 * set_member("b", ["a", "b", "c"])  // succeeds if "b" is in the set
 */
 ...
 * sequence("v:number", 1, 10, 1)
 * // returns 9 generated solutions, 1..9
 * once(sequence("v:number", 1, 10, 1))
 * // returns only first generated solution, 1
 */
 ...
 * collect(["number"], "v:numbers",
 *   sequence("v:number", 1, 10, 1),
 *   // sequence generates 9 indepedent solutions, 1..9 ()
 * )
 * // numbers variable now has the list of numbers
 */
 ...
 * @example
 * and(
 *   eq("v:doc", new doc({"@type": "Person", "name": "John Doe"})),
 ...
 * //returns the number of triples in the local commit graph
 */
 ...
 * @example
 * type_of(literal("string", "xsd:string"), "v:elementType")
 * // elementType is "xsd:string"
 */
