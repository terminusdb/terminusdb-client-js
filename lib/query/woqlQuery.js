...
WOQLQuery.prototype.or = function(...queries) {
    ...
    for (let i = 0; i < queries.length; i++) {
        let index = this.cursor['or'].length
        let onevar = this.qle(queries[i], index)
        if (
            onevar['query']['@type'] == 'Or' &&
            onevar['query']['or']
        ) {
            this.cursor['or'] = onevar['query']['or']
        } else {
...
WOQLQuery.prototype.sub = function(a, b) {
    ...
    this.cursor['parent'] = this.cleanNodeClass(a)
    this.cursor['child'] = this.cleanNodeClass(b)
    return this
}
...
WOQLQuery.prototype.trim = function(untrimmed, trimmed) {
    ...
    this.cursor['untrimmed'] = this.cleanDataObject(untrimmed)
    this.cursor['trimmed'] = this.cleanDataObject(trimmed)
    return this
}
...
WOQLQuery.prototype.eval = function(arith, res) {
    ...
    this.cursor['result'] = this.cleanArithmeticObject(res)
    return this
}
...
WOQLQuery.prototype.isa = function(a, b) {
    ...
    this.cursor['element'] = this.cleanNodeObject(a)
...
WOQLQuery.prototype.like = function(a, b, dist) {
    ...
    this.cursor['left'] = this.cleanDataObject(a)
    this.cursor['right'] = this.cleanDataObject(b)
    if (dist) {
...
WOQLQuery.prototype.less = function(v1, v2) {
    ...
    this.cursor['left'] = this.cleanDataObject(v1)
    this.cursor['right'] = this.cleanDataObject(v2)
    return this
}
...
WOQLQuery.prototype.idgen = function(prefix, inputVarList, outputVar) {
    ...
    this.cursor['base']['data'] = this.cleanDataObject(this.iri(prefix))
...
WOQLQuery.prototype.cast = function(val, type, vb) {
    ...
    this.cursor['type'] = this.cleanNodeObject(type)
...
WOQLQuery.prototype.group_by = function(gvarlist, groupedvar, output, groupquery) {
    ...
    if (typeof gvarlist == 'string') gvarlist = [gvarlist]
        this.cursor['group_by'] = this.varlist(gvarlist)
    if (typeof groupedvar == 'string') {
        this.cursor['template'] = this.varj(groupedvar)
    } else {
        this.cursor['template'] = this.varlist(groupedvar)
    }
