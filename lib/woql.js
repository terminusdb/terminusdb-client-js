 ...
 * @description Filters the query so that only the variables included in [V1...Vn] are returned in the bindings
 ...
 * @param {string} graph - The resource identifier of a graph (e.g. schema/*)
 * @param {WOQLQuery} [query] - WOQL Query object
 * @returns {WOQLQuery}
 */
WOQL.from = function(graph, query) {
    return new WOQLQuery().from(graph, query)
}

/**
 * @description Sets the current output graph for writing output to.
 * @param {string} graph - The resource identifier of a graph (e.g. instance/main) that will be written to
 * @param {WOQLQuery} query - WOQL Query object
 * @returns {WOQLQuery}
 */
WOQL.into = function(graph, query) {
    return new WOQLQuery().into(graph, query)
}
 ...
 * @returns {WOQLQuery}
 */
WOQL.quad = function(subject, predicate, object, graph) {
    return new WOQLQuery().quad(subject, predicate, object, graph)
}
 ...
 * @returns {WOQLQuery}
 */
WOQL.added_quad = function(subject, predicate, object, graph) {
    return new WOQLQuery().added_quad(subject, predicate, object, graph)
}
 ...
 * @returns {WOQLQuery}
 */
WOQL.removed_quad = function(subject, predicate, object, graph) {
    return new WOQLQuery().removed_quad(subject, predicate, object, graph)
}
...
/**
 * @description Deletes a node identified by an IRI or a JSON-LD document
 * @param {string} IRI - the url that identifies the document
 * @param {string} output -
...
/**
 * @description Imports the Target Resource to the variables defined in vars
 * @param {object} asvars - An AS query (or json representation)
 * @param {WOQLQuery} queryResource - description of where the resource is to be got from (WOQL.file, WOQL.remote, WOQL.post)
 * @returns {WOQLQuery}
 */
//KEVIN
 ...
 * @param {WOQLQuery} varsToExp - An AS query (or json representation)
 ...
 * @param {string | integer} source - Source
...
//KEVIN DO YOU CHANGE THE beharior? the second is not true any more? "xsd:integer",
 ...
 * @param {resourceType} [opts] - The format of the resource data {}
 * @returns {WOQLQuery} WOQLQuery
 *
 * @example
...
//KEVIN Deletes any triples that match the rule [S,P,O]
/**
...
//KEVIN THIS IS DESCRIPTION IS WRONG IN THE DOCS
/**
 * @description Deletes any quads that match the rule [Subject, Predicate, Object, Graph]
 * @param {string} subject - The IRI of a triple’s subject or a variable
 * @param {string} predicate - The IRI of a property or a variable
 * @param {string} object - The IRI of a node or a variable, or a literal
 * @param {string} graph - the resource identifier of a graph possible value are schema/{main - myschema - *} | instance/{main - myschema - *}  | inference/{main - myschema - *}
 ...
 */
WOQL.delete_quad = function(subject, predicate, object, graph) {
    return new WOQLQuery().delete_quad(subject, predicate, object, graph)
}
 ...
 * @returns {object} WOQLQuery
 */
WOQL.add_quad = function(subject, predicate, object, graph) {
    return new WOQLQuery().add_quad(subject, predicate, object, graph)
}

/**
 * @memberOf Primitive
 * @description When the subquery is met, the update query is executed
 * @param {WOQLQuery | string} subquery - the condition query
...
//KEVIN se the sub function what this function wants
//check 
/**
 * @memberOf Primitive.Arithmetic
 * Evaluates the passed arithmetic expression and generates or matches the result value
...
//KEVIN CHECK WITH THE DOCS ON_LINE you said 2 numbers but this is infinite number
/**
 * @memberOf Primitive.Arithmetic
...
//KEVIN SEE THE DOCS ONLINE
/**
 * @memberOf Primitive.Arithmetic
 * Subtracts Numbers N1..Nn
 * @param {...string | ...number} - a variable or numeric containing the value that will be subtracted from
 ...
 * @param {WOQLQuery | string} [subquery] - A subquery which will be optionally matched
 ...
 * @param {WOQLQuery | array} subquery -  a subquery or ([string or numeric]) - a list variable, or a list of variables or numeric literals
...
//KEVIN CHECK THE SUB FUNCTION the input can be only a string ??
/**
 * @memberof Primitive
 * Casts the value of Input to a new value of type Type and stores the result in CastVar
 * @param {string} varName - Either a single variable or a literal of any basic type
...
//KEVIN WRONG EXAMPLE IN DOCUMENTATION online
/**
 *
 * @memberof Primitive
 ...
 */
WOQL.group_by = function(gvarlist, groupedvar, resultVarName, subquery) {
    return new WOQLQuery().group_by(gvarlist, groupedvar, resultVarName, subquery)
}
...
WOQL.size = function(resourceId, resultVarName) {
    return new WOQLQuery().size(Graph, resultVarName)
}

//KEVIN this is wrong in the documentation ???
/**
 * @memberof Primitive
 ...
 * @returns {WOQLClient} A WOQLQuery which contains the size expression
 * @example
 * let [tc] = vars("s")
 ...
 */
WOQL.star = function(graph, Subj, Pred, Obj) {
    return new WOQLQuery().star(graph, Subj, Pred, Obj)
}
 ...
 * @param {string} [object] - The IRI of a node or a variable, or a literal
 * @param {string} [graph] - the resource identifier of a graph possible value are schema/{main - myschema - *} | instance/{main - myschema - *}  | inference/{main - myschema - *}
 * @returns {WOQLClient} - A WOQLQuery which contains the pattern matching expression
 * all("mydoc")
 * //will return every triple in the instance/main graph that has "doc:mydoc" as its subject
 */
WOQL.all = function(Subj, Pred, Obj, graph) {
    return new WOQLQuery().all(Subj, Pred, Obj, graph)
}
 ...
 * @param {FuntionType} [chainType] - Optional type of builder function to build  (default is triple)
...
//KEVIN graph is all the type or only instance ??
/**
 * @memberof NoPrimitive.function
 ...
 * @param {string} [graph] 	- Optional Graph resource identifier
 * @returns {WOQLQuery} A WOQLQuery which contains the insert expression
 * @example
 * insert("mydoc", "MyType")
 * //equivalent to add_triple("mydoc", "type", "MyType")
 */
WOQL.insert = function(classId, classType, graph) {
    return new WOQLQuery().insert(classId, classType, graph)
}
 ...
 * @param {string} [graph] Resource String identifying the graph which will be used for subsequent chained schema calls
 * @returns {WOQLQuery} An empty WOQLQuery with the internal schema graph pointes set to Graph
 * @example
 * schema("schema/dev").add_class("X")
 */

WOQL.schema = function(graph) {
    let woql = new WOQLQuery()
    if (graph) woql.graph(graph)
    return woql
}
...
//KEVIN ADD PROPERTY WITHOUT DOMAIN????
/**
 * @memberof NoPrimitive.schema
...
//KEVIN classVarName???? you don't have in documentation
 ...
 * @param {ClassObj} classObj with id, label, description, parent and properties
...
//KEVIN TO BE see the object description
/**
 * @description Inserts data about a class as a json object - enabling a class and all its properties to be specified in a single function
 * @param {ClassObj} classObj with id, label, description, parent and properties
 * @param {string} [schemaGraph] - the resource identifier of a schema graph. The Default value is schema/main
 ...
 * @param {PropertyObj} propObj - json object which contains fields:
 ...
 * @param {string} [graph] - Resource String identifying the graph from which all triples will be removed
...
WOQL.update_quad = function(subject, predicate, newObject, graph) {
    return new WOQLQuery().update_quad(subject, predicate, newObject, graph)
}
 ...
 */
WOQL.value = function(subject, predicate, objValue, graph) {
    return new WOQLQuery().value(subject, predicate, objValue, graph)
}
 ...
 * @returns {WOQLQuery} A WOQLQuery which contains the a quad or a triple Statement
 */
WOQL.link = function(subject, predicate, object, graph) {
    return new WOQLQuery().link(subject, predicate, object, graph)
}
//KEVIN  @param {*} prop
/**
 * @param {woqlClient} - an WoqlClient instance
 * @param {*} prop ??
 ...
 */
WOQL.makeEnum = function(woqlClient, prop, classId, classLabel, classDesc, schemaGraph) {
