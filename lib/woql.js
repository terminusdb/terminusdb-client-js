 ...
 */
 ...
 WOQL.using = function(Collection, Query){return new WOQLQuery().using(Collection, Query)}

 
/**
 * Adds a text comment to a query - can also be used to wrap any part of a query to turn it off
 * @param {comment} arg - text comment
 * @param {Subq} WOQLQuery - subquery that is "commented out"
 ...
 * @return {object} WOQLQuery
 */
WOQL.update = function(JSON){ return new WOQLQuery().update(JSON);	}
 ...
 * @return {object} WOQLQuery
 */
WOQL.idgen = function(prefix, vari, type){	return new WOQLQuery().idgen(prefix, vari, type); }
 ...
 * @return {object} WOQLQuery
 */
WOQL.upper = function(u, l){	return new WOQLQuery().uppder(l, u); }
 ...
 * @param {string} Subj - target subject
 * @param {string} Pred - target predicate
 * @param {string} Obj - target object
 * @return {object} WOQLQuery
 */
WOQL.star = function(GraphIRI, Subj, Pred, Obj){	
	let q = new WOQLQuery();
	Subj = (Subj ? q.cleanSubject(Subj) : "v:Subject");
	Pred = (Pred ? q.cleanPredicate(Pred) : "v:Predicate");
	Obj = (Obj  ? q.cleanObject(Obj) : "v:Object");
	GraphIRI = (GraphIRI ? q.cleanGraph(GraphIRI) : false);
	if(GraphIRI){
		return q.quad(Subj, Pred, Obj, GraphIRI);
	}
	else {
		//console.log(Subj, Pred, Obj);
		return q.triple(Subj, Pred, Obj);
	}
}	
 ...
 * @return {object} WOQLQuery
 */
WOQL.insert = function(Node, Type, Graph){ return new WOQLQuery().insert(Node, Type, Graph) };
 ...
 * @return {object} WOQLQuery
 */
WOQL.query = function(QID){
	if(QID){
		var lib = new WOQLLibrary().fetch(QID)
		if(lib) return lib
	}
