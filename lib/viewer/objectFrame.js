...
ObjectFrame.prototype.getDocumentLinks = function(){
    let vals = []
    let props = this.getProperties("filled")
    for(var i = 0; i<props.length; i++){
        let mprop = this.properties[props[i]]
        for(var k = 0; k<mprop.values.length; k++){
            let dval = mprop.values[k]
            if(dval.isObject() && dval.getDocumentLinks){
                let nvals = dval.getDocumentLinks()
...
ObjectFrame.prototype.saveQuery = function (){
    let q = WOQL.update_object(this.extract())
    this.pathToDoc(q)
    return q
}
...
ObjectFrame.prototype.deleteQuery = function (){
    let q = WOQL.delete_object(this.subjid)
    if(this.parent){
...
DataFrame.prototype.deleteQuery = function (){
    let q = WOQL.delete_triple(this.subject(), this.predicate, this.extract())
    return q
}
