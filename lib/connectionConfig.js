...
/**
 * Setting 4 cursor related state variables at once
 */
ConnectionConfig.prototype.setCursor = function (account, db, repo, ref) {
...
ConnectionConfig.prototype.branchBase = function (action) {
	...
	if(this.branch()) b += `/branch/${this.branch()}`
 ...
 * Generate base branch url consisting of server/action/account/dbid/branchid
 */
ConnectionConfig.prototype.obranchBase = function (action) {
	let b = this.dbBase(action)
	if(this.branch()) b += `/${this.branch()}`
	else b += "/" + this.default_branch_id
	return b
};
