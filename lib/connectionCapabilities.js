...
/**
 * Get the database details list for the get capabilities call
 * @returns {array}
 */
...
ConnectionCapabilities.prototype._role_covers_resource_action = function(role, action, resname) {
    for (var docid in role.capabilities) {
        if (this._capability_covers_resource_action(role.capabilities[docid], action, resname))
            return true
    }
    return false
}

ConnectionCapabilities.prototype._capability_covers_resource_action = function(
    cap,
    action,
    resname,
) {
    ...
    if (scope) {
        ...
        for (var i = 0; i < resources.length; i++) {
            ...
            if (resources[i]['@id'] && resources[i]['@type']) {
                if (
                    resources[i]['@type'] == 'system:SystemDatabase' ||
                    (resources[i]['@type'] == 'system:Database' &&
                        !this.dbdocs[resources[i]['@id']])
                ) {
                    this.dbdocs[resources[i]['@id']] = this._extract_database(resources[i])
                }
                if (
                    resources[i]['@type'] == 'system:Organization' &&
                    !this.orgdocs[resources[i]['@id']]
                ) {
