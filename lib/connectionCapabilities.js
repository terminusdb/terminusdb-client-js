...
ConnectionCapabilities.prototype.extractUserInfo = function(capabilities) {
    ...
    if (capabilities['terminus:agent_name']) info.id = capabilities['terminus:agent_name']['@value']
    if (capabilities['terminus:commit_log_id'])
        info.author = capabilities['terminus:commit_log_id']['@value']
    return info
}
...
ConnectionCapabilities.prototype.author = function() {
    ...
    else return this.user.id + ":ILLUSTRATIVE_UNIQUIFIER or " + this.user.name
}
...
ConnectionCapabilities.prototype.findResourceDocumentID = function(dbid, account) {
    ...
    for (const pred of Object.keys(this.connection)) {
        ...
        if (
            rec['terminus:resource_name'] &&
            rec['terminus:resource_name']['@value'] &&
            rec['terminus:resource_name']['@value'] === testrn
        ) {
            return pred
        }
    }
...
ConnectionCapabilities.prototype.loadConnectionContext = function(ctxt) {
    for (var k in ctxt) {
        if (k == 'doc') this.terminusdb_doc = ctxt[k]
        else this.jsonld_context[k] = ctxt[k]
    }
}

ConnectionCapabilities.prototype.getContextForOutboundQuery = function(woql) {
    let ob = {}
...
/**
 * Same as above, but with mapping to simple json format {}
 */
