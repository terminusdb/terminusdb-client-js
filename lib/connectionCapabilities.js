...
function ConnectionCapabilities() {
    this.user = {}
    this.databases = {} //docid => metadata
    this.organizations = {} //docid => metadata
    this.jsonld_context = {}
...
ConnectionCapabilities.prototype.get_databases = function() {
    let dbs = []
    for(var docid in this.databases){
        if(this.databases[docid].system == false) {
            dbs.push(this._get_db_rec(this.databases[docid]))
        }
    }
    return dbs
}
...
ConnectionCapabilities.prototype.get_database = function(dbid, orgid) {
    for(var docid in this.databases){
        if(this.databases[docid].id == dbid && this.databases[docid].organization == orgid) return this._get_db_rec(this.databases[docid])
        else if(this._is_system_db(dbid) && this.databases[docid].id == dbid) return this._get_db_rec(this.databases[docid])
    }
}
...
ConnectionCapabilities.prototype.get_organization = function(resname) {
    for(var docid in this.organizations){
        if(this.organizations[docid].id == resname) return this.organizations[docid] 
    }
}
    ...
    if(scope){
        ...
        for(var i = 0; i<resources.length; i++){
            ...
            if(resources[i]['@id'] && resources[i]['@type']){
                if(resources[i]['@type'] == "system:SystemDatabase" || (resources[i]['@type'] == "system:Database" && !this.databases[resources[i]['@id']])){
                    this.databases[resources[i]['@id']] = this._extract_database(resources[i])
                }
                if(resources[i]['@type'] == "system:Organization" && !this.organizations[resources[i]['@id']]){
                    this.organizations[resources[i]['@id']] = this._extract_organization(resources[i])
                }
            }
        }
    }
...
ConnectionCapabilities.prototype._extract_resource_name = function(jres) {
    return this._single_rdf_value('system:resource_name', jres)
}

ConnectionCapabilities.prototype._extract_database = function(jres) {
    let db = this._extract_rdf_basics(jres)
    db.id = this._single_rdf_value("system:resource_name", jres)
...
ConnectionCapabilities.prototype._extract_database_organizations = function(){
    for(var docid in this.databases){
        for(var odocid in this.organizations){
            if(this.organizations[odocid].databases.indexOf(docid) != -1){
                this.databases[docid].organization = this.organizations[odocid].id 
            }
        }
    }
}
...
ConnectionCapabilities.prototype._extract_organization = function(jres) {
    ...
    org.id = this._single_rdf_value("system:resource_name", jres)
...
ConnectionCapabilities.prototype._extract_rdf_basics = function(rdf_json){
    ...
    info.description = this._single_rdf_value('rdfs:comment', rdf_json)
    return info
}    
