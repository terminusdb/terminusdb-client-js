 ...
 */
function DispatchRequest(url, action, payload,jwt=null) {
	/*
	...
	*/
	...
	/*
	* if we have the 'terminus:user_key' (for connect at local terminusDB)
	*/
	if (payload && payload['terminus:user_key']) {
		options.headers = { Authorization: `Basic ${btoaImplementation(`admin:${payload['terminus:user_key']}`)}` };
