 ...
 * @param {typedef.CloneDetails} cloneDetails - object describing the clone details info
 * @param {string} newDbId - id of the new cloned database on the local server
 * @param {string} [orgId] - id of the local organization where the database will be cloned
 ...
 */
WOQLClient.prototype.clonedb = function (cloneDetails, newDbId, orgId) {
  orgId = orgId || this.user_organization();
  this.organization(orgId);
  const rc_args = this.prepareRevisionControlArgs(cloneDetails);
...
/**
 * Get the patch of difference between two documents.
 * @param {string} dataVersion - The version (branch or commitid) from which to compare the object
