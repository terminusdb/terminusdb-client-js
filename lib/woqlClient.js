 ...
 * @public
 */
WOQLClient.prototype.connect = function (curl, key, dbid, branchid, jwt) {
	...
	return this.dispatch(CONST.CONNECT,undefined,undefined,jwt)
	.then((response) => {
 ...
 * @public
 */
WOQLClient.prototype.createDatabase = function (details, key,jwt) {
	if(details && details['@id']){
		...
		return this.dispatch(CONST.CREATE_DATABASE, doc,undefined,jwt);
	}
	else {
 ...
 *
 */
WOQLClient.prototype.deleteDatabase = function (key,jwt) {
	...
	const self = this;
	return this.dispatch(CONST.DELETE_DATABASE, opts, undefined,jwt).then(
		(response) => {
 ...
 * The third pargument (igid) is an optional instance graph id - where the document will be written 
 *
 ...
 * @param {string} igid - instance graph identifier
 */
WOQLClient.prototype.createDocument = function (doc, opts, igid) {
	//doc = this.wrapDocumentForAPI(doc, opts);
	let q = WOQL.update_object(doc)
	return this.query(q, opts, igid)
	return this.dispatch(CONST.WOQL_QUERY, doc, igid);
};
 ...
 */
WOQLClient.prototype.getDocument = function (docurl, opts, igid) {
 ...
 * the second argument (opts) is an options json - opts.key is an optional API key
 */
WOQLClient.prototype.updateDocument = function (doc, opts, igid) {
	let q = WOQL.update_object(doc)
	return this.query(q, opts, igid)
	...
	if(doc && doc['@id']){
		doc = this.wrapDocumentForAPI(doc, opts);
		return this.dispatch(CONST.UPDATE_DOCUMENT, doc);
	}
	let errmsg = "Create database parameter error - you should provide a JSON LD document with an @id field";
	return Promise.reject(
		new Error(ErrorMessage.getInvalidParameterMessage(CONST.UPDATE_DOCUMENT, errmsg))
	);
};
 ...
 * the second argument (opts) is an options json - opts.key is an optional API key
 */
WOQLClient.prototype.deleteDocument = function (docurl, opts, igid) {
	let q = WOQL.delete_object(docurl)
	return this.query(q, opts, igid)
	if (docurl && (!this.connectionConfig.setDocument(docurl) || !this.connectionConfig.docid)) {
		return Promise.reject(
 ...
 *
 */
WOQLClient.prototype.query = function (woql, opts, refid,jwt) {
	let q = { 'terminus:query': JSON.stringify(woql) };
	q = this.addOptionsToWOQL(q, opts);
	return this.dispatch(CONST.WOQL_QUERY, q, refid, jwt);
};
...
WOQLClient.prototype.clone = function (refid, newid, jwt) {
	doc = {
		"@type": "terminus:APIUpdate",
		"terminus:resource": refid
	}
	return this.dispatch(CONST.CLONE, doc, newid,jwt);
};

WOQLClient.prototype.fetch = function (repoid,jwt) {
	//doc = this.wrapDocumentForAPI(doc, opts);
	return this.dispatch(CONST.FETCH, {}, repoid,jwt);
};

WOQLClient.prototype.rebase = function (remote_repo_id,jwt) {
	//doc = this.wrapDocumentForAPI(doc, opts);
	return this.dispatch(CONST.REBASE, {}, remote_repo_id,jwt);
};

WOQLClient.prototype.push = function (doc, opts,jwt) {
	doc = this.wrapDocumentForAPI(doc, opts);
	return this.dispatch(CONST.PUSH, doc,jwt);
};

WOQLClient.prototype.branch = function (doc, opts,jwt) {
	doc = this.wrapDocumentForAPI(doc, opts);
	return this.dispatch(CONST.BRANCH, doc,jwt);
};

/*
* this is only locally???
*/
WOQLClient.prototype.createGraph = function (type, gid) {
	gid = gid || "main"
	return this.dispatch(CONST.CREATE_GRAPH, {}, type + "/" + gid,jwt);
};

WOQLClient.prototype.deleteGraph = function (type, gid) {
	gid = gid || "main"
	return this.dispatch(CONST.DELETE_GRAPH, {}, type + "/" + gid);
};
...
WOQLClient.prototype.setDatabaseDefaults = function(dbdoc){
	if(!dbdoc['terminus:default_branch_id']){
		dbdoc['terminus:default_branch_id'] = {"@value": "master", "@type": "xsd:string"}
	}
	return dbdoc;
}
...
WOQLClient.prototype.addKeyToPayload = function (payload, url) {
	/*
	* typeof null is object for javascript
	*/
...
WOQLClient.prototype.db = function (dbid, branchid) {
	if(typeof dbid != "undefined"){
		...
		if(typeof branchid != "undefined") this.connectionConfig.setBranch(branchid)
	}
...
WOQLClient.prototype.getActionURL = function(action, extra) {
	...
	else if(action == CONST.CREATE_DATABASE ){
		//return this.server() + "db"
		return this.server() + encodeURIComponent(this.db());
	}
	else if(action == CONST.DELETE_DATABASE){
		//return this.server() + "db" + encodeURIComponent(this.db());
		return this.server() + encodeURIComponent(this.db());
	}
	else if(action == CONST.GET_SCHEMA || action == CONST.UPDATE_SCHEMA){
	...
	else if(action == CONST.CREATE_DOCUMENT || action == CONST.UPDATE_DOCUMENT ||
	...
	else if(action == CONST.CLASS_FRAME){
	...
	else if(action == CONST.CLONE){
		return this.server() + "clone" + (extra ? "/" + encodeURIComponent(extra) : "");
	}
	else if(action == CONST.PUSH){
		return this.server() + "push/" + encodeURIComponent(this.db());
	}
	else if(action == CONST.WOQL_QUERY){
		//return this.server() + encodeURIComponent(this.db()) + "/woql"
		let base = this.server() + "woql"
		if(this.db() != "terminus") base += "/" + encodeURIComponent(this.db()) // + "/" + encodeURIComponent(this.checkout());
		return base
	}
	else if(action == CONST.FETCH){
		return this.server() + "fetch/" + encodeURIComponent(this.db());
	}
	else if(action == CONST.REBASE){
		return this.server() + "rebase/" + encodeURIComponent(this.db()) + "/" + encodeURIComponent(this.checkout())
	}
	else if(action == CONST.BRANCH){
		return this.server() + "branch/" + encodeURIComponent(this.db()) + "/" + encodeURIComponent(this.checkout())
	}
	else if(action == CONST.CREATE_GRAPH){
		return this.server() + "graph/" + encodeURIComponent(this.db()) + "/" + encodeURIComponent(this.checkout()) + "/" + extra	
	}
	else if(action == CONST.DELETE_GRAPH){
		return this.server() + "graph/" + encodeURIComponent(this.db()) + "/" + encodeURIComponent(this.checkout()) + "/" + extra	
	}
}
 ...
 */
WOQLClient.prototype.dispatch = function (action, payload, extra, jwt) {
	...
	return DispatchRequest(api_url, action, payload,jwt);
};
