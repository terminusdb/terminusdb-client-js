...
/**
 * Retrieves a list of databases (id, organization, label, comment) that the current user has
 * access to on the server. Note that this requires the client to call connect() first.
...
/**
 * Retrieves a list of databases (id, organization, label, comment) that the current user
 * has access to on the server. Note that this requires the client to call connect() first.
...
/**
 * Creates a new branch with a TerminusDB database, starting from the current context
 * of the client (branch / ref)
 * @param {string} newBranchId - local identifier of the new branch the ID of the new
 * branch to be created
 * @param {boolean} [sourceFree] - if the query contains any updates, it should include
 * a textual message describing the reason for the update
 * @returns {Promise}  A promise that returns the call response object, or an Error if rejected.
 * @example
 * client.branch("dev")
 */
