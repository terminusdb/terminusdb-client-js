 ...
 */
WOQLClient.prototype.user_databases = function() {
...
WOQLClient.prototype.user = function() {
    let u = this.connection.get_user()
    return u
}

WOQLClient.prototype.author = function(aname, aid) {
    if (aname) {
        this.connection.user.author = aname
    }
    if (aid) {
        this.connection.user.id = aid
    }
    return this.connection.user.author || this.connection.user.id || this.connectionConfig.local_user()
}
...
WOQLClient.prototype.connect = function(config) {
    ...
    return this.dispatch(CONST.CONNECT, this.server()).then(response => {
...
WOQLClient.prototype.updateDatabase = function(doc) {
    return this.createDatabase(this.db(), doc, this.organization())
}
 ...
 * @return {Promise}
 */
WOQLClient.prototype.deleteDatabase = function(dbid, orgid) {
    orgid = orgid || this.organization()
    this.organization(orgid)
    if (dbid && this.db(dbid)) {
...
WOQLClient.prototype.updateUser = WOQLClient.prototype.createUser
...
WOQLClient.prototype.updateOrganization = WOQLClient.prototype.createOrganization
...
WOQLClient.prototype.getRoles = function(uid, resid, orgid) {
    let doc = {}
    if (uid) {
        doc['agent_name'] = uid
    }
    if (resid) {
        doc['resource_name'] = resid
    }
    if (orgid) {
...
WOQLClient.prototype.updateRoles = function(uid, resid, roles, orgid) {
    let doc = {}
    if (uid) {
        doc['agent_name'] = uid
    }
    ...
    doc['roles'] = roles
    return this.dispatch(CONST.UPDATE_ROLES, this.connectionConfig.rolesURL(), doc)
}
...
WOQLClient.prototype.query = function(woql, commit_msg) {
    ...
    if (woql && woql.json && (!woql.containsUpdate() || commit_msg)) {
        woql.context(this.connection.getContextForOutboundQuery(woql, this.db()))
 ...
 * @param {object} [remote_source_repo] - source repo - {remote: "origin", "remote_branch": "master", "author": "admin", "message": "message"}
 */
 ...
 * @param {object} [remote_source_repo] - source repo - {remote: "origin", "remote_branch": "master", "author": "admin", "message": "message"}
 */
WOQLClient.prototype.fetch = function(remote_source_repo) {
    let rc_args = this.prepareRevisionControlArgs(remote_source_repo)
    if (rc_args && rc_args.remote && rc_args.remote_branch) {
        return this.dispatch(CONST.FETCH, this.connectionConfig.pullURL(), rc_args)
    }
    let errmsg = `Pull parameter error - you must specify a valid remote source and branch to pull from`
    return Promise.reject(new Error(ErrorMessage.getInvalidParameterMessage(CONST.FETCH, errmsg)))
}

/**
 * Pushes changes to the current database / branch to a remote repo
 * @param {object} [remote_target_repo] - target repo - {remote: "origin", "remote_branch": "master", "author": "admin", "message": "message"}
 */
