...
WOQLClient.prototype.jwt = function (jwt, user) {
	if(typeof jwt != "undefined"){
		this.connectionConfig.setJWT(jwt, user)
	}
 ...
 * schema get the database's schema in owl
 * instance get all the database's documents data in owl format
 * inference get all the constraint schema in owl  
 *
 ...
 *
 * @param {string} gtype Type of graph (instance|schema|inference)
 * @param {string} gid TerminusDB Graph ID to update
 * @param {string} turtle is a valid set of triples in turtle format
 * @param {string} commit_msg Textual message describing the reason for the update
 * @return {Promise}
 *
