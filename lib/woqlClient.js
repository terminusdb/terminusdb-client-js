 ...
 * @returns {Promise}  A promise that returns the call response object, or an Error if rejected.
 */
WOQLClient.prototype.dispatch = function (action, apiUrl, payload, getDataVersion, compress = false) {
  if (!apiUrl) {
    return Promise.reject(
      new Error(
...
WOQLClient.prototype.addDocument = function (json, params, dbId, message = 'add a new document', lastDataVersion = '', getDataVersion = false, compress = false) {
  ...
  console.log(`YOU ARE USING COMPRESS IN ADD DOCUMENT TO ${compress}`)
