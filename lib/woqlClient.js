...
WOQLClient.prototype.query = function(woql, commit_msg) {
    ...
    if (woql && woql.json && (!woql.containsUpdate() || commit_msg)) {
        let ctxt = woql.getContext()
        if (!ctxt) woql.context(this.connection.getJSONContext())
...
WOQLClient.prototype.pull = function(remote_source_repo) {
    if (remote_source_repo && remote_source_repo.remote) {
        return this.dispatch(CONST.PULL, this.connectionConfig.pullURL(), remote_source_repo)
    }
    let errmsg = `Pull parameter error - you must specify a valid remote source`
...
WOQLClient.prototype.push = function(remote_target_repo) {
    if (remote_target_repo && remote_target_repo.remote) {
        return this.dispatch(CONST.PUSH, this.connectionConfig.pushURL(), remote_target_repo)
    }
...
WOQLClient.prototype.rebase = function(rebase_source) {
    if (rebase_source && rebase_source.rebase_from) {
        return this.dispatch(
            CONST.REBASE,
