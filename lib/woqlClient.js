 ...
 */
WOQLClient.prototype.query = function(woql, commitMsg, allWitnesses, lastVersion='', getVersion=false) {
    ...
    if (woql && woql.json && (!woql.containsUpdate() || commitMsg)) {
        ...
        if(typeof lastVersion === 'string' && lastVersion !== '') {
            this.customHeaders({'TerminusDB-Data-Version': lastVersion})
        }

        return this.dispatch(CONST.WOQL_QUERY, this.connectionConfig.queryURL(), doql, getVersion)
    }
 ...
 * @returns {Promise}  A promise that returns the call response object, or an Error if rejected.
 */
WOQLClient.prototype.dispatch =  function(action, apiUrl, payload, getVersion) {
    if (!apiUrl) {
        return Promise.reject(
            new Error(
...
WOQLClient.prototype.addDocument = function(json, params, dbId, message="add a new document", lastVersion='', getVersion=false){
    if (dbId) {
        this.db(dbId)
    }
    ...
    return this.dispatch(CONST.POST, this.connectionConfig.documentURL(docParams), json, getVersion)
}
...
WOQLClient.prototype.queryDocument = function(query, params, dbId, branch, lastVersion='', getVersion = false){
    if (dbId) {
        this.db(dbId)
    }
    if(branch){  
    ...
    return this.dispatch(CONST.QUERY_DOCUMENT, this.connectionConfig.documentURL(params), query, getVersion)
}
 ...
 */
//document interface
WOQLClient.prototype.getDocument = function(params,dbId,branch,lastVersion='', getVersion=false){
    if (dbId) {
        this.db(dbId)
    }
    if(branch){  
    ...
    return this.dispatch(CONST.GET, this.connectionConfig.documentURL(params),{}, getVersion)
}
...
WOQLClient.prototype.updateDocument = function(json,params,dbId, message="update document", lastVersion='', getVersion=false){
    const docParams = params || {}
    docParams['author']=this.author()
    docParams['message'] = message
    if (dbId) {
    ...
    return this.dispatch(CONST.PUT, this.connectionConfig.documentURL(docParams),json, getVersion)
}
...
WOQLClient.prototype.deleteDocument = function(params,dbId,message="delete document", lastVersion='', getVersion=false){
    const docParams = params || {}
    let payload = null
    ...
    return this.dispatch(CONST.DELETE, this.connectionConfig.documentURL(docParams),payload, getVersion)
}
/**
