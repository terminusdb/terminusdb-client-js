 ...
 */
function WOQLClient(serverUrl, params=undefined) {
...
/**
 * the current connected server - can only be set by creating a new terminusDB instance
 * @returns {string} 
 */
...
/**
 * Sets/Gets the current organization id
...
/**
 * Gets the current organizzation id
 * @returns {string}
 */
...
WOQLClient.prototype.databases = function(dbList) {
    if (dbList) this.connection.set_databases(dbList)
 ...
 * @param {json} action - structure of the action
 */
WOQLClient.prototype.action = function(action_name, payload) {
    const url = this.api() + "action/" + action_name
