...
describe('woql rules', function () { 
    ...
    it('check direct create WOQL Rule',function(){
    ...
    it('Check Terminus Pattern',function(){
        const woqlRule = new WOQLRule();
        var t = "hello world";
        var x = woqlRule.pattern.testValue(t, t); 
        var y = woqlRule.pattern.testValue(t, [t]); 
        var z = woqlRule.pattern.testValue({"@value": t}, [t]); 
        var c = woqlRule.pattern.testValue({"@value": t}, function(n){ return n == "hello world"}); 
        expect(x).to.equal(true);
        expect(y).to.equal(true);
        expect(z).to.equal(true);
        expect(c).to.equal(true);
    })
    it('Check Terminus Basics',function(){
        const woqlRule = new WOQLRule();
        woqlRule.scope("row").type("xsd:integer").value(">45");
        var t = woqlRule.pattern.testBasics("row", {"@value": 46, "@type": "xsd:integer"});
