...
'use strict';var axios=require('axios'),CONST=require('./const.js'),ErrorMessage=require('./errorMessage');function getOptionsByAction(a,b,c){var d='get';switch(b){case CONST.CONNECT:case CONST.GET_SCHEMA:case CONST.CLASS_FRAME:case CONST.WOQL_SELECT:case CONST.GET_DOCUMENT:c&&(a.params=c);break;case CONST.DELETE_DATABASE:case CONST.DELETE_DOCUMENT:d='delete';break;case CONST.CREATE_DATABASE:case CONST.UPDATE_SCHEMA:case CONST.CREATE_DOCUMENT:case CONST.UPDATE_DOCUMENT:case CONST.WOQL_UPDATE:d='post',a.headers={"Content-Type":'application/json'},a.data=JSON.stringify(c);break;default:return'get';}return d}function DispatchRequest(a,b,c){var d={mode:'cors',redirect:'follow',referrer:'client'},e=getOptionsByAction(d,b,c)||'get';return axios[e](a,d).then(function(a){return a.data}).catch(function(b){console.log(b),Promise.reject(new Error(ErrorMessage.getAPIErrorMessage(a,d)))})}module.exports=DispatchRequest;
